...
from aitviewer.utils.so3 import aa2rot_torch as aa2rot, rot2aa_torch as rot2aa
...
class SMPLLayer(nn.Module, ABC):
    ...
    def skeletons(self):
        ...
        if self.model_type == "smplh":
            hands = parents[
                :,
                self.bm.NUM_BODY_JOINTS
                + 1 : self.bm.NUM_BODY_JOINTS
                + 1
                + 2 * self.bm.NUM_HAND_JOINTS,
            ]
            _all = torch.cat([body, hands], dim=1)
            skeletons["hands"] = hands
        elif self.model_type == "smplx":
            # skip eyes and jaw
            hands = parents[
                :,
                self.bm.NUM_BODY_JOINTS
                + 4 : self.bm.NUM_BODY_JOINTS
                + 4
                + 2 * self.bm.NUM_HAND_JOINTS,
            ]
            hands[hands > self.bm.NUM_BODY_JOINTS] -= 3  # offset by 3 for eyes and jaw
            _all = torch.cat([body, hands], dim=1)
            skeletons["hands"] = hands
        else:
            _all = body
    ...
    def fk_mano(
        ...
        if global_orient is None:
            global_orient = torch.zeros([batch_size, 3]).to(
                dtype=hand_pose.dtype, device=device
            )
        if trans is None:
            trans = torch.zeros([batch_size, 3]).to(
                dtype=hand_pose.dtype, device=device
